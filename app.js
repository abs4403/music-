  ...
  switch (id) {
    case 'truck':
      return `
        ...
        <div class="truck-rig">
          <div class="truck-bunting"></div>
          <div class="truck-cab"></div>
          <div class="truck-trailer">
            <div class="truck-flower f1"></div>
            <div class="truck-flower f2"></div>
          </div>
    ...
    case 'saloon':
      return `
        <div class="saloon-wall"></div>
        <div class="saloon-sign">SALOON</div>
        ...
        <div class="saloon-tool" style="left:12%;top:18%;animation-delay:0s;">✂️</div>
        <div class="saloon-tool" style="left:80%;top:14%;animation-delay:.6s;">🪒</div>
        <div class="saloon-tool" style="left:88%;top:40%;animation-delay:1.2s;">💈</div>
        <div class="saloon-tool" style="left:6%;top:44%;animation-delay:1.8s;">🧴</div>`;
    case 'gym':
      return `
        <div class="gym-neon"></div>
    ...
    case 'romantic':
      return `
        <div class="rom-sky"></div>
        <div class="rom-wave"></div>
        <div class="rom-wave w2"></div>
    ...
    case 'party':
      return `
        <div class="party-sunburst"></div>
        <div class="disco-ball"></div>
        <div class="beam"></div>
        <div class="beam b2"></div>
        <div class="beam b3"></div>
        <div class="confetti" style="left:8%;animation-delay:0s;background:#ffe73d;"></div>
        <div class="confetti" style="left:20%;animation-delay:.8s;background:#00e5ff;"></div>
        <div class="confetti" style="left:35%;animation-delay:1.6s;background:#ff2ec4;"></div>
        <div class="confetti" style="left:50%;animation-delay:.4s;background:#ffe73d;"></div>
        <div class="confetti" style="left:65%;animation-delay:1.2s;background:#00e5ff;"></div>
        <div class="confetti" style="left:78%;animation-delay:2s;background:#ff2ec4;"></div>
        <div class="confetti" style="left:90%;animation-delay:.6s;background:#ffe73d;"></div>
        <div class="spark" style="left:15%;top:20%;animation-delay:0s;"></div>
        <div class="spark" style="left:70%;top:16%;animation-delay:.7s;"></div>
        <div class="spark" style="left:85%;top:34%;animation-delay:1.4s;"></div>
        <div class="spark" style="left:25%;top:38%;animation-delay:.3s;"></div>
        <div class="party-dancers">
          <div class="p-dancer"><div class="arm"></div></div>
          <div class="p-dancer"><div class="arm"></div></div>
          <div class="p-dancer"><div class="arm"></div></div>
          <div class="p-dancer"><div class="arm"></div></div>
          <div class="p-dancer"><div class="arm"></div></div>
...
const players = {};
const currentIndex = {}; // genreId -> which song we're on, tracked ourselves
      ...
      events: {
        onReady: e => {
          if (g.videoIds && g.videoIds.length) {
            currentIndex[g.id] = 0;
            e.target.cuePlaylist({ playlist: g.videoIds, index: 0 });
          }
        },
        onStateChange: e => handleState(g.id, e)
      }
    };
    if (!(g.videoIds && g.videoIds.length)) {
      opts.playerVars.listType = 'playlist';
      opts.playerVars.list = g.playlistId;
    }
...
function handleState(id, e) {
  ...
  // only react to states that actually mean something to the UI —
  // ignore BUFFERING/UNSTARTED/CUED so the icon doesn't flicker while loading
  if (e.data === YT.PlayerState.PLAYING) {
    ...
    if (g && g.videoIds && g.videoIds.length && nowPlaying) {
      ...
      if (idx > -1) {
        currentIndex[id] = idx; // stay in sync even if video changed some other way
        nowPlaying.textContent = g.songTitles[idx];
      }
    }
  ...
  // BUFFERING (3), UNSTARTED (-1), CUED (5): leave the button/reels as they are
}
...
document.querySelectorAll('[data-next]').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    const g = GENRES.find(x => x.id === id);
    const p = players[id];
    if (!p || !p.playVideoAt) return;
    if (g && g.videoIds && g.videoIds.length) {
      const total = g.videoIds.length;
      currentIndex[id] = ((currentIndex[id] ?? 0) + 1) % total;
      btn.closest('.tape-page').querySelector('[data-play]').classList.add('loading');
      btn.closest('.tape-page').querySelector('[data-play]').textContent = '⏳';
      p.playVideoAt(currentIndex[id]);
    } else if (p.nextVideo) {
      p.nextVideo();
    }
  });
...
document.querySelectorAll('[data-prev]').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (g && g.videoIds && g.videoIds.length) {
      const total = g.videoIds.length;
      currentIndex[id] = ((currentIndex[id] ?? 0) - 1 + total) % total;
      btn.closest('.tape-page').querySelector('[data-play]').classList.add('loading');
      btn.closest('.tape-page').querySelector('[data-play]').textContent = '⏳';
      p.playVideoAt(currentIndex[id]);
    } else if (p.previousVideo) {
      p.previousVideo();
    }
  });
});
