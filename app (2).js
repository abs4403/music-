...
document.querySelectorAll('[data-next]').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (g && g.videoIds && g.videoIds.length) {
      const cur = p.getPlaylistIndex ? p.getPlaylistIndex() : 0;
      const nextIdx = (cur + 1) % g.videoIds.length;
      p.playVideoAt(nextIdx);
    } else if (p.nextVideo) {
      p.nextVideo();
    }
  });
...
document.querySelectorAll('[data-prev]').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (g && g.videoIds && g.videoIds.length) {
      const cur = p.getPlaylistIndex ? p.getPlaylistIndex() : 0;
      const prevIdx = (cur - 1 + g.videoIds.length) % g.videoIds.length;
      p.playVideoAt(prevIdx);
